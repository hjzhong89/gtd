  ...
  computed: {
    ...
    pointGroups() {
      ...
      return this.queries.map((q, i) => {
        const points = q.eventids.map(eventid => {
          const incident = that.incidents[eventid]
          return {
    ...
    activeEvents() {
      if (this.focused) {
        const focusPoints = d3.selectAll(`.point[data-name]==${this.focused}`);
        console.log(focusPoints)
      }
    }, // When focused, incidents in the focused country
  },
  async created() {
    this.center = {
      x: this.width / 2,
